...
def getRightPOSCAR(path):
        # The latest version of makeStr.py writes zero concentration. Example:
        # Co Ni Ti
        # ...
        # 1 0 3
        # which is different from previous versions. Now getting the right
        # POTCARs and POSCAR will be in charge of "fixing_POSCARs.cpp" which should
        # be already compiled at this point by the script "scriptsCoNiTi.sh" that
        # called this python file.
        #getPOTCAR(vaspFile) << it won't be necessary anymore.
        os.system("mv vasp.* POSCAR") #renames the vaspFile got in a previous step.
        os.system("./fixing_POSCARs.out") # fixes POSCAR, and get POTCAR. It will
        # produce "fixedPOSCAR"
        os.system("mv POSCAR backUpPOSCAR")
        os.system("mv fixedPOSCAR POSCAR")

        run = "mv POSCAR " +  path + "/"
        os.system(run)
        run = "mv backUpPOSCAR " +  path + "/"
        os.system(run)
